...
class TestPlayerCharacterFethriLevel20(unittest.TestCase):
    def setUp(self):
        self.fethri = pc_playground.create_fethri(20)
    ...
    def test_level(self):
        self.assertEqual(self.fethri.level, 20)

    def test_background(self):
        self.assertEqual(self.fethri.background_name, 'Noble')

    def test_proficiency_bonus(self):
        self.assertEqual(self.fethri.proficiency_bonus, 6)
    ...
    def test_ability_scores(self):
        ability_scores = self.fethri.ability_scores
        self.assertEqual(ability_scores['STR'].score, 10)
        self.assertEqual(ability_scores['DEX'].score, 20)
        self.assertEqual(ability_scores['CON'].score, 12)
        self.assertEqual(ability_scores['INT'].score, 20)
        self.assertEqual(ability_scores['WIS'].score, 11)
        self.assertEqual(ability_scores['CHA'].score, 16)

    def test_armor_class(self):
        self.assertEqual(self.fethri.armor_class, 16)

    def test_initiative(self):
        self.assertEqual(self.fethri.initiative, 5)
    ...
    def test_max_hit_points(self):
        self.assertEqual(self.fethri.max_hit_points, 123)

    def test_total_hit_die(self):
        self.assertEqual(self.fethri.total_hit_dice, {'d8': 20})

    def total_hit_dice_prettified(self):
        self.assertEqual(self.fethri.total_hit_dice_prettified, '20d8')
    ...
    def test_skills_by_ability(self):
        ...
        stealth = skillz['INT']['History']
        self.assertEqual(stealth.ability, 'INT')
        self.assertEqual(stealth.is_proficient, True)
        self.assertEqual(stealth.expertise, False)
        self.assertEqual(stealth.modifier, 11)

        stealth = skillz['DEX']['Stealth']
        self.assertEqual(stealth.ability, 'DEX')
        self.assertEqual(stealth.is_proficient, True)
        self.assertEqual(stealth.expertise, True)
        self.assertEqual(stealth.modifier, 17)

        investigation = skillz['INT']['Investigation']
        self.assertEqual(investigation.ability, 'INT')
        self.assertEqual(investigation.is_proficient, True)
        self.assertEqual(investigation.expertise, True)
        self.assertEqual(investigation.modifier, 17)
    ...
    def test_vocation_features(self):
        self.assertEqual(len(self.fethri.vocation_features), 17)
    ...
    def test_calculate_weapon_bonuses(self):
        ...
        rapier_bonus = weapon_bonuses['rapier']
        self.assertEqual(rapier_bonus['attack_bonus'], 11)
        self.assertEqual(rapier_bonus['attack_type'], 'DEX')
        self.assertEqual(rapier_bonus['damage'], '1d8 piercing + 5 + 10d6 [Sneak Attack]')
